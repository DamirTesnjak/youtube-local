...
export default function ProgressDisplayItem({
  progressInfo,
  clientId,
}: {
  ...
  return (
    <div
      id={progressInfo.videoName}
      className='mt-3 p-3 rounded-sm border-1 border-gray-300 shadow-lg'
    >
      <span className='pr-3 font-semibold'>{progressInfo.videoName}</span>
      {progressInfo.completed && (
        <span>
          <button
            className='w-50 pt-1 pb-1 pl-1 pr-1 rounded-sm bg-blue-950 text-white font-light font-stretch-semi-expanded cursor-pointer hover:bg-blue-600'
            onClick={() =>
              window.open(
                `http://localhost:4001/video?file=${progressInfo.path}`,
                '_blank',
              )
            } // openFile(progressInfo.path as string)}
          >
      ...
      {progressInfo.completed && (
        <video className='mt-2' width='320' height='240' controls>
          <source
            src={`http://localhost:4001/video?file=${progressInfo.path}`}
            type='video/mp4'
          />
      ...
      {progressInfo.audioMessage && (
        <div id='audioTrack' className='flex flex-col mb-2'>
          <span className='text-sm'>
            Audio {progressInfo.audioMessage} | {progressInfo.audioMB}{' '}
          </span>
      ...
      {progressInfo.videoMessage && (
        <div id='videoTrack' className='flex flex-col mb-2'>
          <span className='text-sm'>
            Video {progressInfo.videoMessage} | {progressInfo.videoMB}
          </span>
      ...
      {progressInfo.canceledDownload && (
        <p className='mt-4 text-sm'>Download cancelled</p>
      )}
    </div>
  );
}
